...
namespace code_gen {
	...
	int handle_expression(std::vector<Parser::expression_type>& base_expression, std::vector<instruction>& instructions, scope* local_scope, int& mem_count);
	...
	void handle_function_call(Parser::instruction& ins, std::vector<instruction>& instructions, scope* local_scope, int& mem_count) {
		...
		for(int i = 0; i < functions.size(); i++) { //Find the function
			if(ins.identifier.name == functions[i].name) {
				index = i;
				break;
			}
		}
		...
		if(index == -1) { //Function not found
			print_error("Unknown function called: " + ins.identifier.name + " - on line: " + std::to_string(ins.identifier.line));
			return;
		}else{ //Function found
			argument_mem_locations.push_back(index); //Push the function index
			for(auto& i : ins.arguments) { //Handle all the arguments
	...
	int handle_expression(std::vector<Parser::expression_type>& base_expression, std::vector<instruction>& instructions, scope* local_scope, int& mem_count) {
		std::vector<Parser::expression_type> postfix_expression = infix_to_postfix(base_expression);
				...
				if(std::get<0>(postfix_expression[i]).type == Lexer::lexer_identifier || std::get<0>(postfix_expression[i]).type == Lexer::lexer_value) {
					...
					if(a.index() == 1) {
						...
						if(free_reg == -1) {
							...
							instructions.push_back({0x5, {1}}); //Allocate memory to store value from first reg
							instructions.push_back({0x4, {0, (unsigned int)mem_index_a, 0}}); //Store the reg value
							if(std::get<0>(a).type == Lexer::lexer_value) {
					...
					if(b.index() == 1) {
						...
						if(free_reg == -1) {
							...
							instructions.push_back({0x5, {1}}); //Allocate memory to store value from first reg
							instructions.push_back({0x4, {1, (unsigned int)mem_index_b, 0}}); //Store the reg value
							if(std::get<0>(b).type == Lexer::lexer_value) {
	...
	void process_variable_definition(Parser::tree_node* node, std::vector<instruction>& instructions, scope* local_scope, int& mem_count) {
		int var_mem_location = handle_expression(node->ins.expression, instructions, local_scope, mem_count);
		local_scope->variables.push_back({node->ins.identifier.name, 0, var_mem_location});
	}
	...
	void process_function_definition(Parser::tree_node* node) {
		...
		for(auto& i : node->ins.def_arguments) { //Handle function arguments
			new_function.function_scope->variables.push_back({i.identifier, 0, new_function.mem_count});
	...
	void process_variable_change(Parser::tree_node* node, std::vector<instruction>& instructions, scope* local_scope, int& mem_count) {
		int old_var_mem_location = find_variable(local_scope, node->ins.identifier.name);
	...
	void process_if_statement(Parser::tree_node* node, std::vector<instruction>& instructions, scope* local_scope, int& mem_count) {
		int condition_mem_location = handle_expression(node->ins.expression, instructions, local_scope, mem_count);
	...
	void process_return(Parser::tree_node* node, std::vector<instruction>& instructions, scope* local_scope, int& mem_count) {
		int return_value_mem_location = handle_expression(node->ins.expression, instructions, local_scope, mem_count);
